...
def TechnologyParametersFunc(use_flags, TechsToTest):
    #rv = {"data" : {data}, "use_flags" : {use_flags}}
    data = {'tech_flag' : TechsToTest,
            'use_tech' : 0,
            'measure_losses' : 0,
            'dump_bills' : 0,
            'measure_capacitor' : 0,
    ...
    for TechToTest in TechsToTest:
        ...
        # Co-Simulated Real Time Prices
            ...
            use_flags["use_commercial"] = 1;
            # These will include recorders/collectors/dumps
            use_flags["use_billing"] = 3; # hourly billing based on market
            use_flags["use_emissions"] = 1;
            use_flags["use_capacitor_outtages"] = 1;
            data["measure_losses"] = 1; 
            data["dump_bills"] = 1;
            ...
            # adds in the market
            use_flags["use_market"] = 4; # co-simulated real time prices
            
            # adds in customer/technology interactions
            data["use_tech"] = 1;  # existing option that puts in price-based thermostats
                                   # may need to tweak behavior based on use_market flag of 4
            
            #Prints stats at bottom of GLM
            data["include_stats"] = 1;

            # Adds in meter consumption
            data["meter_consumption"] = 2; # advanced meter (instead of electromechanical)
    ...
    if (use_flags["use_market"] != 0):
        ...
        data["market_info"] = { 'name': 'Market_1', 
                                'period': 900,
                                'avg_name': "my_avg",
                                'std_name': "my_std",
