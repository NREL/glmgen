...
def create_glm_object_dictionary(glm_dict,glm_object,glm_parameters):
	...
	def create_object(glm_props, object):
...
######### ALL OBJECTS WE WISH TO CREATE SHOULD HAVE A STORED (KEY : VALUE) IN THIS DICTIONARY!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		if object not in glm_objects:
			raise RuntimeError("Unsupported GridLAB-D object given: {:s}.".format(object))
        
		# Make sure glm_parameters is the appropriate size
		if len(glm_props) != len(glm_objects[object]):
			raise RuntimeError("Incorrect number of parameters specified for the {:s} object.".format(object))
		...
		for x in range(len(glm_props)):
			if glm_props[x] != None:
				...
				glm_dict[key][glm_objects[object][x]] = glm_props[x]
	# End create_object()
